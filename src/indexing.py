                ...
                if first_char == '[':
                        ...
                        if in_string:
                            ...
                            if char == '"':
                                ...
                                if brackets_depth == 0:
                                    ...
                                    current_object = ""
                                    next_char = f.read(1)
                                    while next_char and next_char.isspace() or next_char == ',':
                                        next_char = f.read(1)
                            ...
                            if current_time - last_progress_time > 30:
                                ...
                                print(f"Processed {count} documents in {elapsed:.2f} seconds ({count/elapsed:.2f} docs/sec)")
                    ...
                    for line in f:
                        ...
                        try:
                            ...
                            print(f"Skipping line due to JSON decode error: {json_err} - Line: {line[:100]}...") # Print start of line
                        except Exception as e:
    ...
    try:
        ...
        for key, value in list(processed_doc.items()): # Iterate over a copy of items
            if isinstance(value, dict) and '$oid' in value:
                processed_doc[key] = value['$oid'] # Replace the dict with just the ID string

        # Special handling for artist-members.json
        if filename == "artist-members.json" or os.path.basename(filename) == "artist-members.json":
            # Handle the members array
            if 'members' in processed_doc and isinstance(processed_doc['members'], list):
                for member in processed_doc['members']:
                    ...
                    if 'birthDate' in member:
                        ...
                        elif isinstance(member['birthDate'], str) and re.match(r'^\d{4}$', member['birthDate']):
        ...
        if index_name == 'albums':
            ...
            if 'dateRelease' in processed_doc:
                if not processed_doc['dateRelease']:
                    ...
                    date_match = re.match(r'^(\d{4}-\d{2}-\d{2})', processed_doc['dateRelease'])
                    if date_match:
            ...
            if 'publicationDateAlbum' in processed_doc:
                if not processed_doc['publicationDateAlbum']:
                    ...
                    year_match = re.search(r'(\d{4})', processed_doc['publicationDateAlbum'])
                    if year_match:
            ...
            if 'publicationDate' in processed_doc:
                if not processed_doc['publicationDate']:
                    ...
                    date_match = re.match(r'^(\d{4}-\d{2}-\d{2})', processed_doc['publicationDate'])
                    if date_match:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--subset', type=int, help='Number of documents to index per file (for debugging)')
    parser.add_argument('--file', type=str, help='Only process a specific file (e.g., "song.json")')
    parser.add_argument('--recreate-indices', action='store_true', help='Delete and recreate indices')
    parser.add_argument('--skip-existing', action='store_true', help='Skip files with an existing index')
    ...
    if args.file:
        if args.file in FILE_TO_INDEX_MAP:
            ...
            print(f"Warning: Specified file '{args.file}' not found in configuration. Available files: {list(FILE_TO_INDEX_MAP.keys())}")
            exit()
    else:
    ...
    for filename, index_name in files_to_process.items():
        ...
        if args.skip_existing and es.indices.exists(index=index_name):
            ...
            if count_response['count'] > 0:
                print(f"Skipping {filename} because index '{index_name}' already has {count_response['count']} documents.")
                continue
        ...
        try:
            ...
            if errors:
                ...
                for i, error in enumerate(errors[:5]):
                    print(f"Error {i+1}:")
                    print(f"  Operation: {error.get('index', {}).get('_op_type', 'unknown')}")
                    print(f"  Document ID: {error.get('index', {}).get('_id', 'unknown')}")
                    print(f"  Error type: {error.get('index', {}).get('error', {}).get('type', 'unknown')}")
                    print(f"  Error reason: {error.get('index', {}).get('error', {}).get('reason', 'unknown')}")
            else:
                print(f"Bulk indexing for {filename}: Successfully indexed {success} documents with no errors.")
                print(f"Indexing speed: {success/elapsed:.2f} docs/sec")
